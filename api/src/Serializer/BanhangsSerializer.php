...
namespace Joomla\Component\Banhangs\Api\Serializer;
 ...
 * @since  4.0.0
 */
class BanhangsSerializer extends JoomlaSerializer
{
	...
	public function languageAssociations($model)
	{
		...
		foreach ($model->associations as $association)
		{
			$resources[] = (new Resource($association, $serializer))
				->addLink('self', Route::link('site', Uri::root() . 'api/index.php/v1/banhangs/banhangs/' . $association->id));
		}
	...
	public function category($model)
	{
		...
		$resource = (new Resource($model->catid, $serializer))
			->addLink('self', Route::link('site', Uri::root() . 'api/index.php/v1/banhangs/categories/' . $model->catid));
