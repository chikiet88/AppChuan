...
namespace Joomla\Component\Banhangs\Site\Model;
...
/**
 * Banhangs Component Banhang Model
 *
 * @since  1.5
 */
class BanhangModel extends ItemModel
{
	/**
	 ...
	 * @since   1.6
	 */
	protected $_context = 'com_banhangs.banhang';
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_banhangs')) && (!$user->authorise('core.edit', 'com_banhangs')))
		{
	...
	/**
	 * Method to get banhang data.
	 *
	 * @param   integer  $pk  The id of the banhang.
	 *
	...
	{
		$pk = (int) $pk ?: (int) $this->getState('banhang.id');
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query = $db->getQuery(true)
					...
					->from($db->quoteName('#__banhangs', 'a'))
				...
				if ($data === null)
				{
					throw new \Exception(Text::_('COM_BANHANGS_ERROR_FEED_NOT_FOUND'), 404);
				}
	...
	/**
	 * Increment the hit counter for the banhang.
	 *
	...
	public function hit($pk = 0)
	{
		...
		if ($hitcount)
		{
			$pk = (!empty($pk)) ? $pk : (int) $this->getState('banhang.id');

			$table = $this->getTable('Banhang', 'Administrator');
			$table->hit($pk);
		}
