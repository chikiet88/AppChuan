...
namespace Joomla\Component\Banhangs\Administrator\Service\HTML;
...
class AdministratorService
{
	 ...
	 *
	 * @param   int  $banhangid  The item id to search associations
	 *
	 * @return  string  The language HTML
	 *
	 * @throws  \Exception  Throws a 500 Exception on Database failure
	 */
	public function association($banhangid)
	{
		// Defaults
		$html = '';

		// Get the associations
		if ($associations = Associations::getAssociations('com_banhangs', '#__banhangs', 'com_banhangs.item', $banhangid))
		{
			...
			// Get the associated banhang items
			$db = Factory::getDbo();
			...
			$query
				...
				->from($db->quoteName('#__banhangs', 'c'))
				...
				->bind(':id', $banhangid, ParameterType::INTEGER);
			$db->setQuery($query);
			...
			if ($items)
			{
				...
				foreach ($items as &$item)
				{
					...
					{
						$text    = $item->lang_code;
						$url     = Route::_('index.php?option=com_banhangs&task=banhang.edit&id=' . (int) $item->id);
						$tooltip = '<strong>' . htmlspecialchars($item->language_title, ENT_QUOTES, 'UTF-8') . '</strong><br>'
							. htmlspecialchars($item->title, ENT_QUOTES, 'UTF-8') . '<br>' . Text::sprintf('JCATEGORY_SPRINTF', $item->category_title);
						$classes = 'badge bg-secondary';

						$item->link = '<a href="' . $url . '" class="' . $classes . '">' . $text . '</a>'
							. '<div role="tooltip" id="tip-' . (int) $banhangid . '-' . (int) $item->id . '">' . $tooltip . '</div>';
					}
					else
					{
