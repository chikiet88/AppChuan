...
/**
 * The banhang controller for ajax requests
 *
 * @since  3.9.0
 */
...
{
	/**
	 * Method to fetch associations of a banhang
	 *
	 * The method assumes that the following http parameters are passed in an Ajax Get request:
	 * token: the form token
	 * assocId: the id of the banhang whose associations are to be returned
	...
	public function fetchAssociations()
	{
		...
		else
		{
			...
			$associations = Associations::getAssociations('com_banhangs', '#__banhangs', 'com_banhangs.item', (int) $assocId);

			unset($associations[$excludeLang]);

			// Add the title to each of the associated records
			$banhangsTable = $this->factory->createTable('Banhang', 'Administrator');

			foreach ($associations as $lang => $association)
			{
				$banhangsTable->load($association->id);
				$associations[$lang]->title = $banhangsTable->name;
			}
